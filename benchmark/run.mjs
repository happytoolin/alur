...
function fastCases(fixturePaths) {
        ...
        commands: [
          {
            name: "pm",
            bin: "nr",
            args: ["-C", fixturePaths[pm.fixtureKey], "noop"],
            env: { ALUR_FAST_MODE: "false" },
          },
          {
            name: "fast",
            bin: "nr",
        ...
        commands: [
          {
            name: "pm",
            bin: "nr",
            args: ["-C", fixturePaths[pm.fixtureKey], "hooks"],
            env: { ALUR_FAST_MODE: "false" },
          },
          {
            name: "fast",
        ...
        commands: [
          {
            name: "pm",
            bin: "node",
            args: ["-C", fixturePaths[pm.fixtureKey], "run", "noop"],
            env: { ALUR_FAST_MODE: "false" },
          },
          {
            name: "fast",
  ...
  cases.push({
    id: "npm_nex_hello",
    group: "nex",
    case: "nex hello --flag (npm local bin)",
    commands: [
      { name: "pm", bin: "nex", args: ["-C", fixturePaths.npm, "hello", "--flag"], env: { ALUR_FAST_MODE: "false" } },
      { name: "fast", bin: "nex", args: ["-C", fixturePaths.npm, "hello", "--flag"], env: { ALUR_FAST_MODE: "true" } },
    ],
    requiredBins: ["npm"],
  });
...
function runHyperfineCase({ repoRoot, caseDef, runs, warmups, rawOutputPath, commands }) {
  ...
  for (const command of commands.slice(1)) {
    relativeToFirstMean[command.name] = participants[baseline].mean / participants[command.name].mean;
    relativeToFirstMedian[command.name] = participants[baseline].median / participants[command.name].median;
  }
...
function printTrackSummary(payload, format) {
  ...
  if (payload.track === "runtime") {
    ...
    if (competitors.length <= 1) {
      ...
      const separator = "  ";
      const summaryWidth = widths.case
  ...
  for (const [name, value] of Object.entries(payload.summary.geometric_mean_relative_to_first)) {
    lines.push(
      `geometric mean relative to ${payload.results[0]?.baseline ?? "baseline"} (${name}): ${value.toFixed(2)}x`,
    );
  }
...
function makeTrackTable(payload) {
  if (payload.track === "runtime") {
    ...
    for (const row of payload.results) {
      lines.push(
        `| ${row.case} | ${formatMs(row.participants.alur.mean)} | ${formatMs(row.participants.bun.mean)} | ${
          formatMs(row.participants.deno.mean)
        } |`,
      );
    }
  ...
  if (competitors.length <= 1) {
    ...
    for (const row of payload.results) {
      lines.push(
        `| ${row.case} | ${formatMs(row.participants[baseline].mean)} | ${
          formatMs(
